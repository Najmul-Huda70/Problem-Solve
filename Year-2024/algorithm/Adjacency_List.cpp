...
cin.tie(0);
 ...
 /*
 int arr[n]; 
 => ekane array te int type mane bojai 
    array er protita index e  int type data ase;
vector<int> arr[n];
=> ekane array te "vector<int>"  type mane bojai
   array er protita  index e ekti kore vector ase.
 */
 vector<int> arr[n];
 while(e--)
 {
    int a,b;
    cin>>a>>b;
  arr[a].push_back(b); //eta mane bojai a er shate b aconnected
  //arr[b].push_back(a); // eta mane bojai b er shate a connected
 }
for(int i=0;i<n;i++)
{
    cout<<"arr["<<i<<"] = ";
    for(auto x:arr[i])  //arr[0] ekta vector
{
    cout<<x<<" ";
}
cout<<endl;
